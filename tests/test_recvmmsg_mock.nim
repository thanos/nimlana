...
proc addPacket*(receiver: MockBatchReceiver, data: seq[byte], source: TransportAddress) =
  ## Add a packet to the mock receiver queue
  receiver.packets.add(BatchPacket(
    data: data,
    source: source,
    timestamp: 0.0, # Will be set by ingestor
  ))
...
type
  BatchTPUIngestor* = ref object
    ingestor*: TPUIngestor # Composition instead of inheritance
    batchReceiver*: MockBatchReceiver
    batchMode*: bool

proc newBatchTPUIngestor*(port: Port, batchReceiver: MockBatchReceiver): BatchTPUIngestor =
...
suite "recvmmsg Mock Tests - Edge Cases":
  ...
  test "Batch TPU ingestor with error handling":
    ...
    receiver.addPacket(@[], address) # Empty (handlePacket skips it, but it's still in batch)
